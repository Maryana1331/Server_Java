package com.example.Advanced_server_Ostrogotskaya.constants;
...
public enum ErrorCodes {
    ...
    FILE_UPLOAD_FAILED(55, ValidationConstants.EXCEPTION_HANDLER_NOT_PROVIDED);

    public static Map<String, Integer> typeErrors = new HashMap<>();
    ...
    static {
        typeErrors = new HashMap<>();
        for (ErrorCodes enumItem : values()) {
            typeErrors.put(enumItem.getMessage(), enumItem.getCode());
        }
    }

    public static Integer getCodeByMessage(String message) {
        return typeErrors.get(message);
    }
